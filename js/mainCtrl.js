(function() {
   ...
   function mainCtrl($scope, itunesService){
      ...
      var vm = this;
      //
      vm.getSongData = _.debounce(getSongData, 300, {
         "leading": false,
         "trailing": true
      });

     //This is setting up the default behavior of our ng-grid. The important thing to note is the 'data' property. The value is 'songData'. That means ng-grid is looking for songData on $scope and is putting whatever songData is into the grid. This means when you make your iTunes request, you'll need to get back the information, parse it accordingly, then set it to songData on the scope -> $scope.songData = ...
     vm.gridOptions = {
         data: 'songData',
         height: '110px',
         ...
         columnDefs: [
           ...
           {field: 'Album', displayName: 'Album Name'},
           {field: 'AlbumArt', displayName: 'Album Art', width: '110px', cellTemplate: '<div class="ngCellText" ng-class="col.colIndex()"><img src="{{row.getProperty(col.field)}}"></div>'},
           {field: 'Type', displayName: 'Type'},
           {field: 'TrackLength', displayName: 'Track Length'},
           {field: 'SongPrice', displayName: 'Song Price'},
           {field: 'AlbumPrice', displayName: 'Album Price'},
         ]
     };
     ...
     //Also note that that method should be returning a promise, so you could use .then in this function.
       ...
       function getSongData() {
         itunesService.getArtistName(vm.artist).then(function(results) {
           console.log("getSongData results: ", results);
         //   $scope.songData = results;
            $scope.songData = results.map(function(song) {
               return{
                 ...
                 TrackLength: song.trackTimeMillis,
                 Type: song.kind,
                 SongPrice: song.trackPrice,
